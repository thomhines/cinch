...
class cinch {
	...
	*----------------------------------------------------------------------*/
	...
	private $file_array = array('css' => array(), 'js' => array());
	private $error = "";
	...
	*----------------------------------------------------------------------*/
	...
	function run($_settings) {
		...
		$settings = $this->setDefaultSettings($_settings);
		...
		$cachefile_timestamp = $this->hasCacheFile($cachefile, $file_array[$settings['type']]);
		...
		// Else, build new cache files, CSS then JS
			...
			$file_output = $this->processFileList($file_array[$settings['type']]);
	...
	function sortFiles($file_list_array, $package = "") {
		...
		foreach($file_list_array as $file) {
			...
			if(substr($file, 0, 1) == "[") {
				...
				$library_files = $this->getExternalLibraryFiles($library);
				$this->sortFiles($library_files, $library);
			} else {
				if($this->getFileType($file) == 'js' && $package) $file_array['js'][$package][] = $file;
	
				else if($this->getFileType($file) == 'css' && $package) $file_array['css'][$package][] = $file;
				
				else if($this->getFileType($file) == 'js') $file_array['js'][] = $file; // || $library_info['type'] == 'js') $_GET['type'] = 'js';
	
				else if($this->getFileType($file) == 'css') $file_array['css'][] = $file;
			}
			$x++;
		}
	} 
		...
		foreach($type_file_array as $key => $file) { // combine files
			if(is_array($file)) {
				...
				foreach($file as $sub_file) {
					$content .= $this->processFile($sub_file);
				}
			}
			else $content .= $this->processFile($file);	
		}
		return $content;
	}
	...
	function processFile($file) {
		...
		// LOAD FILE CONTENTS
		$temp_content = $this->loadFile($file);
		$file_type = $this->getFileType($file);
		
		
		// IF SASS, PROCESS AND CONVERT TO CSS
		if(substr($file, -5) == '.scss' || substr($file, -5) == '.sass') $temp_content = $this->convertSASS($temp_content, $file);
		
		// IF LESS, PROCESS AND CONVERT TO CSS
		if(substr($file, -5) == '.less') $temp_content = $this->convertLESS($temp_content, $file);
	
		// IF COFFEESCRIPT, PROCESS AND CONVERT TO JS
		if(substr($file, -7) == '.coffee') $temp_content = $this->convertCoffee($temp_content);
	...
	function loadFile($file) {
		...
		if(substr($file, 0, 4) == 'http') {
			...
			if(!$content = @file_get_contents($file)) $this->recordError("'".$file."' is not a valid remote file.");
		}
	...
	*/
		...
		// Local file
			...
			if(!is_file(SITE_ABS_PATH.$file)) $this->recordError("'".$file."' is not a valid file.");
			else {
				if(!$handle = fopen(SITE_ABS_PATH.$file, "r")) $this->recordError("There was an error trying to open '".$file."'");
				else {
					$content = "";
					if(filesize(SITE_ABS_PATH.$file) == 0) $this->recordError("'".$file."' is an empty file.");
					elseif(!$content = fread($handle, filesize(SITE_ABS_PATH.$file))) $this->recordError("There was an error trying to open '".$file."'.");
					fclose($handle);
				}
			}
		}
	...
	function loadLibraryInfo($file) {
		...
		if(!$library) {
			$this->recordError("'".$file."' is not a valid library name.");
			return;
		}
	...
	*/
	...
	function loadExternalLibrary($library_url) {
		...
		if(ini_get("allow_url_fopen") == 0) {
			if(ini_get("allow_url_fopen") == 0) $this->recordError("Your server does not allow for access to remote libraries. You may need to upload the file to your web server manually.");
		}
	...
	*/
	...
	function getExternalLibraryFiles($library) {
		...
		if(!file_exists(CINCH_ABS_PATH.$package_path)) $this->downloadRepo($package);
		...
		if(!$package_info) {
			$this->recordError("The package '" . $package[0] . "' doesn't have a valid bower.json file.");
			return false;
		}
		...
		if(!$library_files) $this->recordError("The package '" . $package[0] . "' hasn't stated which of its files to include in its bower.json.");
	...
	function getRepoUrl($package) {
		...
		if(file_exists($bower_local)) $bower_directory = json_decode(file_get_contents($bower_local));
			...
			if($handle = fopen($bower_local, 'w')) {
				if(fwrite($handle, $bower_directory_contents) === FALSE) {
					$this->recordError("Cannot write to local bower directory file. Make sure the permissions on your server are set to allow write access to the 'cache' folder.");
				}
			}
			else $this->recordError("Cannot open bower directory file. Make sure the permissions on your server are set to allow write access to the 'cache' folder.");
			if($handle) fclose($handle);
		}
		...
		if(!$repo_root_url) {
			$this->recordError("Cannot find package '" . $package[0] . "' in bower package list.");
			return false;
		}
	...
	*/
	...
	function downloadRepo($package) {
		$repo_url = $this->getRepoUrl($package);
		...
		if($repo_url) {
			...
			if ($zip->open($zip_file) === TRUE) {
				...
				$this->recordError("Cannot open zip file.");
			}
		}
	}
	...
	function convertSASS($src, $file) {
		...
		try {
			...
			$this->recordError("Sass error in $file: " . $e->getMessage() . ".");
			return;
		}
		return $css;
	}
	...
	function convertLESS($src, $file) {
		...
		try {
			...
			if($settings['debug']) $this->recordError("Less error in $file: " . $e->getMessage() . ".");
			return;
		}
		return $css;
	}
	...
	function printCacheFile($cachefile) {
		...
		if(!is_file($cachefile)) {
			$this->$this->recordError("Cache file does not exist.");
			return;
		}
		if(!$handle = fopen($cachefile, "r")) {
			$this->$this->recordError("There was an error trying to open cache file: '".$cachefile."'.");
			//exit;
		} else {
...
// This allows cinch to run as a function. It will automatically separate and process a mixed set of web files
